...
public class ManageBook {
    ...
    public static void main(String[] args) {
        ...
        // 2) 가격 변경 (3번째 도서: 인덱스 2 -> '빠삐용'이 아니라, 샘플은 7번째 도서 변경을 보여줌)
        // 샘플처럼 '작별하지않는다'(인덱스 6)를 변경해볼게.
        Publication target = publications[6];
        int before = target.getPrice();
        System.out.println("==== 가격 변경 ====");
        System.out.println(target.getTitle() + " 변경 전 가격: " + money.format(before) + "원");
        modifyPrice(target); // 타입별 할인
    ...
    // 타입별로 다른 할인율 적용
    // Magazine: 40% 할인(= 60%), Novel: 20% 할인(= 80%), ReferenceBook: 10% 할인(= 90%)
